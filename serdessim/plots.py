...
class Plotter:
    ...
    def _prepare_figure(self, title: str) -> plt.Figure:
        fig = plt.figure(figsize=self.config.figsize, dpi=self.config.dpi)
        fig.suptitle(title)
        return fig

    def eye_density(self, artifacts: SimulationArtifacts) -> plt.Figure:
    ...
    def ber_plots(self, result: SimulationResult) -> plt.Figure:
    ...
    def pdf_plots(self, artifacts: SimulationArtifacts) -> plt.Figure:
    ...
    def _maybe_save(self, figure: plt.Figure, name: str) -> None:
